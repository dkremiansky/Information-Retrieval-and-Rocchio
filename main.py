...
def calc_accuracy(test_set, classifier, cosin = False):
    correct = 0.0
    total = len(test_set.keys())
    for key in test_set:
        real = test_set[key][-1]
        predicted = classifier.predict(test_set[key][0:-1], cosin)
        if real == predicted:
            correct += 1.0
...
if __name__ == '__main__':
    ...
    test_set, _ = data.build_set("tfidf", test_file_name)
    classifier = rocchio_classifier.RocchioClassifier(train_set)
    print("tfidf with cosine similarity:", '{:.3f}'.format(calc_accuracy(test_set, classifier, True)))
